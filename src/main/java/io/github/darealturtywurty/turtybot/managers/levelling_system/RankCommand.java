...
@RegisterBotCmd
public class RankCommand implements GuildCommand {
    ...
    public RankCommand() {
        try {
            ...
            this.usedFont = Font.createFont(Font.TRUETYPE_FONT,
                    TurtyBot.class.getResourceAsStream("/fonts/Code New Roman.otf")).deriveFont(12f);
    ...
    public static BufferedImage cutoutImageMiddle(final BufferedImage image, final int baseWidth,
            final int baseHeight, final int cornerRadius) {
    ...
    public List<OptionData> getOptions() {
        return List.of(new OptionData(OptionType.USER, "member", "Member to get the rank card for", false));
    }

    @Override
    public void handle(final CoreCommandContext ctx) {
        final OptionMapping memberOption = ctx.getEvent().getOption("member");
        Member member = null;
        if (memberOption == null) {
            member = ctx.getMember();
        } else {
            member = memberOption.getAsMember() == null ? ctx.getMember() : memberOption.getAsMember();
        }

        ctx.getEvent().deferReply().addFile(makeRankCard(member), "rank_card.png").mentionRepliedUser(false)
                .queue();
    }
    ...
    private File makeRankCard(final Member member) {
        ...
        try {
            ...
            final BufferedImage base = ImageIO
                    .read(TurtyBot.class.getResourceAsStream("/levels/background.png"));
            final BufferedImage outline = ImageIO
                    .read(TurtyBot.class.getResourceAsStream("/levels/outline.png"));
            ...
            if (card.backgroundImage.isBlank() || member.getTimeBoosted() == null && !member.isOwner()
                ...
                final var bgBuf = new BufferedImage(base.getWidth(), base.getHeight(),
                        BufferedImage.TYPE_INT_ARGB);
            ...
            final var bgBuffer = new BufferedImage(base.getWidth(), base.getHeight(),
                    BufferedImage.TYPE_INT_ARGB);
            ...
            if (!card.xpOutlineImage.isBlank() && (member.getTimeBoosted() != null || member.isOwner()
                ...
                final BufferedImage xpBarOutline = makeRoundedCorner(
                        ImageIO.read(new URL(card.xpOutlineImage)), 20);
            ...
            if (!card.xpEmptyImage.isBlank() && (member.getTimeBoosted() != null || member.isOwner()
                    || member.getGuild().getIdLong() == 819294753732296776L)) {
                final BufferedImage xpBarEmpty = makeRoundedCorner(ImageIO.read(new URL(card.xpEmptyImage)),
                        20);
            ...
            if (!card.xpFillImage.isBlank() && (member.getTimeBoosted() != null || member.isOwner()
                    || member.getGuild().getIdLong() == 819294753732296776L)) {
                final BufferedImage xpBarFill = makeRoundedCorner(ImageIO.read(new URL(card.xpFillImage)),
                        20);
            ...
            if (!card.avatarOutlineImage.isBlank() && (member.getTimeBoosted() != null || member.isOwner()
                ...
                avatarOutGraphics
                        .setClip(new Ellipse2D.Float(0, 0, userAvatar.getWidth(), userAvatar.getHeight()));
                avatarOutGraphics.drawImage(ImageIO.read(new URL(card.avatarOutlineImage)), 0, 0,
                        userAvatar.getWidth(), userAvatar.getHeight(), null);
                avatarOutGraphics.dispose();
                graphics.drawImage(avatarOut, 40, 33, userAvatar.getWidth() + 30, userAvatar.getHeight() + 30,
                        null);
