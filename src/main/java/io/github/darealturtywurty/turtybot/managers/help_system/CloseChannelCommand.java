...
public class CloseChannelCommand implements GuildCommand {
    ...
    public void handle(final CoreCommandContext ctx) {
        ...
        if (!channel.getParent().getName().toLowerCase().contains("support") || !BotUtils
                .isModerator(ctx.getGuild(), ctx.getMember())
                && !channel.getTopic().split("\n")[0].toLowerCase().trim().equalsIgnoreCase(user.getId()))
            return;

        final var confirmButton = Button.danger("confirm._." + user.getName(), "Confirm");
        final var cancelButton = Button.success("cancel._." + user.getName(), "Cancel");
