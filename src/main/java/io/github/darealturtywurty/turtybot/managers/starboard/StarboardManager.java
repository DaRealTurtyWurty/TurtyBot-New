...
import io.github.darealturtywurty.turtybot.util.core.StarboardUtils;
...
public class StarboardManager extends ListenerAdapter {
        ...
        showcases.retrieveMessageById(showcaseInfo.originalMessageID).queue(showcaseMsg -> {
            if (showcaseMsg == null) {
                starboard.retrieveMessageById(showcaseInfo.starboardMessageID).queue(starboardMsg -> {
                    ...
                    authorID.set(Long.parseLong(
                            starboardMsg.getEmbeds().get(0).getAuthor().getProxyIconUrl().split("/")[4]
                                    .toLowerCase().trim()));
                });
                return;
            }
    ...
    public static void updateMessage(final Guild guild, final long messageID, final int stars,
            final long starboardMsgID) {
        final var guildInfo = getOrCreateGuildInfo(guild);
        ...
        CoreBotUtils.GUILDS.put(guild.getIdLong(), guildInfo);
    ...
    public void onGuildMessageDelete(final GuildMessageDeleteEvent event) {
        ...
        if (alreadyExists && showcaseInfo.getStars() < StarboardUtils.getStageStar(event.getGuild(), 1)) {
            ...
            guildInfo.showcaseInfos.remove(event.getMessageIdLong());
            guildInfo.userStarStats.get(getShowcaseUserID(guild, showcaseInfo)).otherStars += showcaseInfo
                    .getStars();
        }
    }
    ...
    public void onGuildMessageReactionRemove(final GuildMessageReactionRemoveEvent event) {
        ...
        if (stars >= StarboardUtils.getMinimumStars(guild)) {
            starboard.retrieveMessageById(showcaseInfo.starboardMessageID)
                    .queue(msg -> msg
                            .editMessage(
                                    getStarEmoji(guild, stars) + " " + stars + " " + showcases.getAsMention())
                            .queue());
            return;
        }
