...
public class Inventory {
    ...
    public boolean addItem(final InventoryItem item, @Nullable final TextChannel channel) {
        ...
        if (!doesntExist) {
            Constants.LOGGER.warning("You cannot add Item: \"" + item.name + "\" to "
                    + this.member.getUser().getName() + "#" + this.member.getUser().getDiscriminator() + "("
                    + this.member.getIdLong() + ")" + "'s inventory because they already have that item!");
            return false;
        }

        this.items.add(item);
        if (channel != null) {
            channel.sendMessage("Congrats " + this.member.getEffectiveName() + ", Item: \"" + item.name
                    + "\" of type (" + item.itemType.name + ") has been added to your inventory! 🎉").queue();
        }
    ...
    public boolean removeItem(final InventoryItem item, @Nullable final TextChannel channel) {
        ...
        if (doesntExist) {
            Constants.LOGGER.warning("You cannot remove Item: \"" + item.name + "\" from "
                    + this.member.getUser().getName() + "#" + this.member.getUser().getDiscriminator() + "("
                    + this.member.getIdLong() + ")" + "'s inventory because they do not have that item!");
            return false;
        }

        this.items.add(item);
        if (channel != null) {
            channel.sendMessage("Oh dear " + this.member.getEffectiveName() + ", Item: \"" + item.name
                    + "\" of type (" + item.itemType.name + ") has been removed from your inventory! 😢")
                    .queue();
        }
        Constants.LEVELLING_MANAGER.updateInventory(this.member);
        return true;
