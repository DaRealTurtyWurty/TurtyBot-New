...
public class LevellingManager extends ListenerAdapter {
	...
	private void randXp(final Message message, final Member member) {
		...
		if (member.getTimeBoosted() != null) {
			...
			final var months = Float.parseFloat(
					decimalFormat.format((member.getTimeBoosted().toInstant().toEpochMilli() - System.currentTimeMillis())
		...
		if (getUserCooldown(member) <= 0) {
			...
			if (newLevel > level) {
				message.getChannel()
						.sendMessage("Congrats " + member.getAsMention() + " you levelled up to level: " + newLevel + "! 🎉")
						.queue();
				if (newLevel % 5 == 0) {
	...
	private void sortLeaderboard(final Guild guild) {
		...
		Stream.of(oldBoard.toArray(new Document[0]))
				.sorted((doc1, doc2) -> Integer.compare(doc2.getInteger("XP"), doc1.getInteger("XP"))).forEach(lb::add);
	}
		...
		for (final var collection : this.database.listCollectionNames()) {
			...
			for (final Document document : this.levels.get(guild).find(Filters.exists("MemberID"))) {
				...
				for (final JsonElement elem : inv) {
					card.inventory.addItem(InventoryItem.parse(elem.getAsJsonObject(), "MongoDB").build(), null);
				}
