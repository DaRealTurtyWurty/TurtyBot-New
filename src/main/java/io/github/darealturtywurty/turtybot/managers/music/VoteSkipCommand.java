...
public class VoteSkipCommand implements GuildCommand {
    ...
    public void handle(final CoreCommandContext ctx) {
        ...
        if (player.getPlayingTrack() != null
                && MusicManager.getMusicManager(ctx.getGuild()).scheduler.getQueue().peek() != null) {
            ctx.getChannel()
                    .sendMessage("A vote-skip has been created. React with ⏭ if you want to skip (0/5).")
                    .queue(msg -> msg.addReaction("⏭").queue());
            CoreBotUtils.GUILDS.get(ctx.getGuild().getIdLong()).skips.add(new Skip(ctx.getGuild().getIdLong(),
                    ctx.getChannel().getIdLong(), ctx.getChannel().getLatestMessageIdLong()));
        }
    }
