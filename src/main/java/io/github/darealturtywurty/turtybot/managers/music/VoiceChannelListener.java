...
public class VoiceChannelListener extends ListenerAdapter {
	...
	protected static final Set<Skip> SKIPS = new HashSet<>();
	...
	public void onGuildMessageReactionAdd(final GuildMessageReactionAddEvent event) {
		...
		final Optional<Skip> oSkip = SKIPS.stream()
				.filter(skip -> skip.guildId == event.getGuild().getIdLong()
						&& skip.channelId == event.getChannel().getIdLong() && skip.messageId == event.getMessageIdLong())
				.findFirst();
		if (oSkip.isPresent()) {
	...
	public void onGuildVoiceLeave(final GuildVoiceLeaveEvent event) {
		...
		if (event.getGuild().getAudioManager().getConnectedChannel() != null
				&& event.getGuild().getAudioManager().getConnectedChannel().getIdLong() == event.getChannelLeft().getIdLong()
