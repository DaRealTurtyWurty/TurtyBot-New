...
import io.github.darealturtywurty.turtybot.managers.levelling_system.InventoryItem.Type;
import io.github.darealturtywurty.turtybot.util.Constants;
import io.github.darealturtywurty.turtybot.util.WeightedRandomBag;
...
public final class ItemRegistry {
	...
	public static List<InventoryItem> get(final String name) {
		return REGISTRY.values().stream().filter(i -> i.name.equalsIgnoreCase(name)).collect(Collectors.toList());
	}
	...
	public static void register(final String loc) {
		final JsonObject object = Constants.GSON
				.fromJson(readString(TurtyBot.class.getResourceAsStream("/items/" + loc + ".json")), JsonObject.class);
	...
	public static InventoryItem scaledRandom() {
		...
		final List<InventoryItem> items = REGISTRY.values().stream().filter(item -> item.rarity == rarities.getRandom())
				.collect(Collectors.toList());
		try {
			return items.get(Constants.RANDOM.nextInt(items.size() - 1));
