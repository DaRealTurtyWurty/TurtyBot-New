...
public class UserWarns {
    ...
    public void addWarn(final Guild guild, final Member warner, final String reason) {
        ...
        this.member.getUser().openPrivateChannel()
                .queue(channel -> channel.sendMessageEmbeds(dmWarnEmbed.build()).queue());

        if (getNumberWarns() == BotUtils.getMuteThreshold(guild)) {
            MuteCommand.muteMember(guild, warner, this.member, null, "Reached Server Mute Threshold!",
                    9000000);
    ...
    public EmbedBuilder getFormattedWarns(final Guild guild) {
        ...
        this.warns.forEach((uuid, warnInfo) -> {
            ...
            embed.addField(
                    "UUID: " + uuid.toString(), "Reason: " + warnInfo.right + "\nWarned By: "
                            + warner.getAsMention() + "\nDate-time: " + DATE_FORMAT.format(warnInfo.middle),
                    false);
        });
    ...
    public boolean removeWarn(final Guild guild, final Member warnRemover, final String strUUID) {
        ...
        if (this.member != null
                && (warnRemover.getIdLong() == this.userID || !warnRemover.canInteract(this.member)))
            return false;
        ...
        if (this.warns.containsKey(uuid)) {
            ...
            final var removeWarnEmbed = new EmbedBuilder().setColor(Color.GREEN)
                    ...
                    .addField("Date:", DATE_FORMAT.format(warn.middle), false)
                    .addField("Reason:", warn.right, false).setTimestamp(Instant.now());
            loggingChannel.sendMessageEmbeds(removeWarnEmbed.build()).queue();

            guild.retrieveBanById(this.userID).queue(ban -> {
                if (getNumberWarns() < BotUtils.getBanThreshold(guild) && ban != null) {
                    System.out.println(ban);
                    UnbanCommand.unbanUser(guild, warnRemover, this.userID, null,
                            "Below guild maximum warn count!");
                }
            }, error -> {
            });
