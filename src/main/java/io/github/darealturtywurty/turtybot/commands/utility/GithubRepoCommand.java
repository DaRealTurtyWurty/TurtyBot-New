...
public class GithubRepoCommand implements IGuildCommand {
	...
	private static Repository getDetails(final JsonObject from) {
		...
		return new Repository(name, authorName, url, description, language, defaultBranch, createdAt, updatedAt, license,
				stars, forks, watchers, openIssueCount, estimateSize, isFork, isArchived, isDisabled);
	}
	...
	private static Color languageToColor(final String language) throws IOException {
		...
		return master.has(language) ? Color.decode(master.get(language).getAsJsonObject().get("color").getAsString())
				: Color.WHITE;
	}
	...
	public void handle(final CommandContext ctx) {
		...
		try {
			...
			if (repo != null) {
				...
				embed.addField("Created At:", DateFormat.getDateTimeInstance().format(parseDateTime(repo.creationDate)),
						false);
			...
			ctx.getMessage().reply("A repository with this name could not be found!").mentionRepliedUser(false).queue();
